...
async function loadUsers(){
  ...
  if(!users || users.length===0){
    users = [
      ...
      { id: uid(), username:'owner', password:'owner123', role:'owner', name:'Owner / Investor' }
    ];
...
/* ==================== NAV / RENDER ==================== */
const NAV_ITEMS = [
  {id:'dashboard',   label:'Dashboard',          ic:'🏠', roles:['admin','owner']},
  ...
  {id:'laporan',     label:'Laporan',            ic:'🧾', roles:['admin','owner']},
  ...
  document.getElementById('sidebarFoot').innerHTML = `
    <div class="user-chip">
      ...
      <div class="info">
        ...
        <div class="urole">${sess.role==='admin' ? '🔐 Admin — dapat mengedit' : '👁️ Owner/Investor — lihat saja'}</div>
      </div>
    </div>
...
function viewOnlyBanner(){
  return `<div class="viewonly-banner"><span>👁️</span> Anda masuk sebagai Owner/Investor — halaman ini hanya untuk dilihat, tidak dapat mengedit data.</div>`;
}
...
async function renderDashboard(main){
  ...
  main.innerHTML = `
    ...
    <div class="hero">
      <img src="image/gerobak.jpg" alt="Gerobak ${escapeHTML(state.meta.businessName)}">
      <div class="hero-content">
      <h2>${escapeHTML(state.meta.businessName)}</h2>
        <div class="eyebrow">Fresh Drinks, Anytime, Anywhere</div>
        <p>${escapeHTML(HARI[new Date().getDay()])}, ${formatDateLabel(today)} — begini ringkasan bisnismu hari ini.</p>
      </div>
    </div>
    ...
    <div class="stat-grid">
      <div class="stat-card" style="border-left-color:var(--green)">
        ...
        <div class="stat-sub">${todayCups} cup · ${todayEntries.length} transaksi${todayHasNight? ' · 🌙 Night Market':''}</div>
      </div>
    ...
    <div class="row" style="align-items:stretch;">
      <div class="card chart-card" style="flex:1.4; min-width:320px;">
        <h3>📈 Pemasukan vs Pengeluaran Harian — ${formatMonthLabel(m)}</h3>
        <div class="chart-wrap"><canvas id="dailyChart"></canvas></div>
      </div>
      <div class="card chart-card" style="flex:1; min-width:280px;">
        <h3>📊 Tren Bulanan (6 Bulan Terakhir)</h3>
    ...
    <div class="card" style="margin-bottom:22px;">
      <div class="row-between" style="margin-bottom:8px;">
        ...
        <span class="badge badge-outline">Reguler saja · Night Market &amp; staff dapur tidak dihitung</span>
      </div>
      ...
      <form id="incomeForm">
        ...
        <div class="seg-control" id="locSeg">
          ...
          <label class="seg-opt night ${location==='nightmarket'?'active':''}" data-loc="nightmarket">
            <input type="radio" name="location" value="nightmarket" ${location==='nightmarket'?'checked':''}> 🌙 Night Market
          </label>
        </div>
...
async function renderVarian(main){
  main.innerHTML = `
    ...
    <div class="card" style="margin-bottom:22px; max-width:560px;">
      <form id="varForm">
        <div class="form-grid">
          ...
          <div class="field">
            <label>Harga Night Market</label>
    ...
    <div class="table-wrap" style="max-width:560px;"><table>
      <tr><th>Varian</th><th class="num">🚚 Harga Reguler</th><th class="num">🌙 Harga Night Market</th><th></th></tr>
      ${state.variants.map(v=> `
        <tr>
...
async function renderLaporan(main, presetMonth){
  ...
  main.innerHTML = `
    ...
    <div class="receipt" style="margin-bottom:30px;">
      ...
      <div class="receipt-row"><span class="l">🌙 Night Market</span><span class="mono">${formatUSD(byLoc.nightmarket||0)}</span></div>
    ...
    <div class="report-section no-print">
      ...
      <div class="row" style="align-items:flex-start;">
        <div style="flex:1; min-width:260px;">
          ...
          <div class="table-wrap"><table>
            <tr><th>Varian</th><th class="num">Reguler</th><th class="num">Night Market</th></tr>
...
async function renderAkun(main){
  ...
  main.innerHTML = `
    ...
    <p class="section-sub">Buat akun login untuk Admin (dapat mengedit) atau Owner/Investor (hanya melihat Dashboard &amp; Laporan).</p>
      ...
      <form id="userForm">
        <div class="form-grid">
          ...
          <div class="field">
            <label>Peran</label>
            <select id="uRole">
              <option value="owner">👁️ Owner / Investor (lihat saja)</option>
    ...
    <div class="table-wrap" style="max-width:640px;"><table>
      ...
      ${users.map(u=> `<tr>
        ...
        <td><span class="acc-role-pill ${u.role==='admin'?'acc-role-admin':'acc-role-owner'}">${u.role==='admin'?'Admin':'Owner/Investor'}</span></td>
        <td style="white-space:nowrap;">
